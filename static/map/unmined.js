...
class Unmined {
    ...
    createMarkersLayer(markers) {
        var features = [];
        var defaultLabelMaxZoom = typeof this.#options.defaultZoom === 'number' ? Math.max(this.#options.defaultZoom + 1, 1) : 1;
        var dotStyleCache = {};
        var self = this;

        function createDotStyles(item, showLabel) {
            var color = item.pinColor || '#d93025';
            var radius = item.dotRadius || 6;
            var label = item.label || item.text || item.title || '';
            var cacheKey = [color, radius, showLabel ? 1 : 0, label].join('|');
            if (dotStyleCache[cacheKey]) return dotStyleCache[cacheKey];
            ...
            if (showLabel && label) {
                styles.push(new ol.style.Style({
                    text: new ol.style.Text({
                        text: label,
                        font: item.font || '600 13px Segoe UI, Arial, sans-serif',
                        offsetX: item.offsetX || 0,
                        offsetY: item.offsetY || -18,
                        fill: new ol.style.Fill({ color: item.textColor || 'rgba(17,24,39,0.62)' }),
                        stroke: new ol.style.Stroke({ color: item.textStrokeColor || 'rgba(255,255,255,0.92)', width: item.textStrokeWidth || 3 })
                    })
                }));
            }

            dotStyleCache[cacheKey] = styles;
            return styles;
        }
        ...
        for (var i = 0; i < markers.length; i++) {
            ...
            feature.set('markerConfig', item);

            if (item.markerType !== 'dot') {
        ...
        var vectorLayer = new ol.layer.Vector({
            ...
            style: function(feature) {
                var item = feature.get('markerConfig');
                if (item && item.markerType === 'dot') {
                    var zoom = self.olMap && self.olMap.getView ? self.olMap.getView().getZoom() : undefined;
                    var labelMinZoom = typeof item.labelMinZoom === 'number' ? item.labelMinZoom : defaultLabelMaxZoom;
                    var dotMinZoom = typeof item.dotMinZoom === 'number' ? item.dotMinZoom : -2;
                    if (typeof zoom === 'number' && zoom < dotMinZoom) {
                        return null;
                    }
                    var showLabel = typeof zoom === 'number' ? zoom >= labelMinZoom : false;
                    return createDotStyles(item, showLabel);
                }
                return feature.getStyle();
            }
        });
        vectorLayer.set('lososinskLayerType', 'markers');
        return vectorLayer;
    }
